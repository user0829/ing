$(function(){
    ...
    $("#subSite").click(function(){
        ...
        if(siteInfo=='' || masterIP=='' || port=='' || remark==''){
            ...
            var data='{"siteInfo":"'+siteInfo+'","masterIP":"'+masterIP+'","port":"'+port+'","remark":"'+remark+'"}';
            data=encodeURI(data);
            $.ajax({
                type:'post',
                url:''+data,
                ...
                success:function(data){
                    if(data.errCode==1){
                        ...
                        setTimeout(function(){
                            $("#formHintSuccess").hide();
                            $("#siteTab").append("<tr><td style='padding: 5px 70px;'>"+siteInfo+"</td><td style='padding: 5px 70px;' class='dbaNum' valueNum='"+masterIP+"'>"+masterIP+"</td><td style='padding: 5px 70px;'>"+port+"</td><td style='padding: 5px 70px;'>"+remark+"</td><td style='padding: 5px 70px;'><a href='###' onclick='deleteColony(this)'>删除</a></td></tr>");
                            $("#siteInfo").val("");
                            $("#masterIP").val("");
                            $("#port").val("");
                            $("#remark").val("");
    ...
    $("#subColony").click(function(){
        ...
        if(siteName=='' || startIP=='' || endIP=='' || gateway=='' || mask=='' || vlan==''){
            ...
            var data='{"siteName":"'+siteName+'","startIP":"'+startIP+'","endIP":"'+endIP+'","gateway":"'+gateway+'","mask":"'+mask+'","vlan":"'+vlan+'"}';
            data=encodeURI(data);
            ...
            $.ajax({
                ...
                success:function(data){
                    if(data.errCode==1){
                        ...
                        setTimeout(function(){
                            $("#formHintSuccess").hide();
                            $("#colonyTab").append("<tr><td style='padding: 5px 45px;'>"+siteName+"</td><td style='padding: 5px 45px;' >"+startIP+"</td><td style='padding: 5px 45px;'>"+endIP+"</td><td style='padding: 5px 45px;'>"+gateway+"</td><td style='padding: 5px 45px;' class='nodeIP' valueNum='"+mask+"'>"+mask+"</td><td style='padding: 5px 45px;'>"+vlan+"</td><td style='padding: 5px 45px;'><a href='###' onclick='deleteColony(this)'>删除</a></td></tr>");
                            $("#siteName").val("");
                            $("#startIP").val("");
                            $("#endIP").val("");
                            $("#gateway").val("");
...
function deleteSite(_this){
    var SiteText=$(_this).parents('td').parents('tr').children('.dbaNum').attr('valueNum');
    var currentObject=$(_this).parents('td').parents("tr");
    var data='{"siteIP":"'+SiteText+'"}';
    data=encodeURI(data);
    $.ajax({
        type:'post',
        url:basePath+'/managerInfo/deleteInfo.do?data='+data+"&flag=5",
